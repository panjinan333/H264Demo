...
-(id)initWithVideo:(NSString *)moviePath
{
    ...
    //先計算出全部的frame
    //frameNumber = [self countVideoFrameNumber:moviePath];
    ...
    if(avformat_open_input(&pFormatCtx, [moviePath cStringUsingEncoding:NSASCIIStringEncoding], NULL, 0)!=0)
    {
    ...
    {
        av_log(NULL, AV_LOG_ERROR, "Can not open video decoder\n");
        goto initError;
	}
    ...
    //NSLog(@"duration:%lld",pFormatCtx->streams[videoStream]->duration);
...
//因計算過video實際的frame數量,所以必須再重新讀取video file一次 - new ffmpeg 20150102
-(int) countVideoFrameNumber:(NSString *) moviePath
{
    AVCodec         *pCodec;
    ...
    while(av_read_frame(pFormatCtx, &packet)>=0)
    {
        ...
        //解析完packet就要free，不然packet會佔住記憶體
        av_free_packet(&packet);
    }
    
    NSLog(@"Total frame number:%d",number);
...
{    
	...
	//int64_t targetFrame = timeBase.num * seconds / (int64_t)((double)timeBase.den);
...
-(void)convertFrameToRGB
{
	sws_scale(img_convert_ctx, (void *)pFrame->data, pFrame->linesize, 0, pCodecCtx->height, picture.data, picture.linesize);
}
...
-(void)savePPMPicture:(AVPicture)pict width:(int)width height:(int)height index:(int)iFrame
{
	...
	fileName = [DocumentPathClass documentsPath:[NSString stringWithFormat:@"image%04d.ppm",iFrame]];
    // Open file
